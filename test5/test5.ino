...
//double stp;
//double stp;  
long PathCycle, PathCycles, PathCnts,  PathCnt, Ctr, pCycle;
long Timer1Clk;
...
signed char sLUT[2][2]; //= {1,1,2,0}; //'set count sign: -1=0,0=1,1=2
...
void Check_pLUT()
{
  ...
  //      Serial.println(Ctr);
        ...
        if (Ctr != PathCnt)
           //Serial.println("error"); ;//printf("Ctr != PathCnt\n");
           badplutflag = 1;
        Ctr = 0;
    }
...
void setup()
{
    ...
    //    'sLUT: lookup table to set count to + or - sign
    sLUT[0][0] = 0; //= {1,1,2,0}; //'set count sign: -1=0,0=1,1=2
    sLUT[0][1] = 0;
    sLUT[1][0] = 1;
    sLUT[1][1] = -1;
...
//        pCycle = 0;
        ...
        for (pCycle = 1; pCycle< 1001; pCycle++)
        {
            ...
            if (1)
            {
                ...
                eCts0 += sLUT[pLUT[pCtsA0][pCycle]][pCtsN0];      //  'update motor counters
                eCts1 += sLUT[pLUT[pCtsA1][pCycle]][pCtsN1];
                eCts2 += sLUT[pLUT[pCtsA2][pCycle]][pCtsN2];
                eCts3 += sLUT[pLUT[pCtsA3][pCycle]][pCtsN3];
            ...
            //if (vLUT[pLUT[pCycle][pCtsA4]][pLUT[pCycle][pCtsA5]][pLUT[pCycle][pCtsA6]][pLUT[pCycle][pCtsA7]])
            {
                ...
                eCts4 += sLUT[pLUT[pCtsA4][pCycle]][pCtsN4];      //  'update motor counters
                eCts5 += sLUT[pLUT[pCtsA5][pCycle]][pCtsN5];
                eCts6 += sLUT[pLUT[pCtsA6][pCycle]][pCtsN6];
                eCts7 += sLUT[pLUT[pCtsA7][pCycle]][pCtsN7];
            }
        }
        runloop = 0;
        time2 = micros();
...
//     Check_pLUT();
     delay(3000);
     Serial.print("Plut check = ");
     Serial.println(badplutflag);
